...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <form method="post" action="${pageContext.request.contextPath}/clientes">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div class="form-group">
                        ...
                        <select id="tipoDocumento" name="tipoDocumento" required>
                            ...
                            <option value="F" <c:if test="${cliente.tipoDocumento == 'F'}">selected</c:if>>CPF (Pessoa F�sica   )</option>
                            <option value="J" <c:if test="${cliente.tipoDocumento == 'J'}">selected</c:if>>CNPJ (Pessoa Jur�dica)</option>
                        </select>
                    </div>
                ...
                <h3 style="color: #667eea; margin-bottom: 15px;">Endere�o</h3>
                ...
                <div style="text-align: right;">
                    ...
                    <button type="submit" class="btn btn-primary"><c:choose><c:when test="${not empty cliente}">Salvar Altera��es</c:when><c:otherwise>Cadastrar Cliente</c:otherwise></c:choose></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        // Fun��o para aplicar a m�scara de CPF ou CNPJ
        function mascaraDocumento(campo) {
            let valor = campo.value.replace(/\D/g, ''); // Remove tudo o que n�o � d�gito
        ...
        // Fun��o para aplicar a m�scara de Telefone (com 9� d�gito)
        function mascaraTelefone(campo) {
            let valor = campo.value.replace(/\D/g, ''); // Remove tudo o que n�o � d�gito
            
            if (valor.length > 11) {
                valor = valor.substring(0, 11); // Limita a 11 d�gitos para evitar problemas com a m�scara
            }

            valor = valor.replace(/^(\d{2})(\d)/g, '($1) $2'); // Coloca par�nteses em volta dos dois primeiros d�gitos
            
            if (valor.length <= 14) { // Telefone com 8 d�gitos (ex: (00) 0000-0000)
                valor = valor.replace(/(\d{4})(\d)/, '$1-$2');
                campo.maxLength = 14;
            } else { // Telefone com 9 d�gitos (ex: (00) 00000-0000)
        ...
        // Fun��o para aplicar a m�scara de CEP
        ...
        // Inicializa as m�scaras se o campo j� tiver valor (caso de edi��o)
